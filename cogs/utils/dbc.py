...
class Player:
    def __init__(self, bot, data, user):
        ...
        try:
            ...
            self.trophy_oid = None
        try:
            self.trophy_rod_level = int(data[6])
        except Exception:
            self.trophy_rod_level = None
        try:
            self.review_message_id = int(data[8])
        except Exception:
            self.review_message_id = None
    ...
    async def update(self, user):
        ...
        cur = await self.bot.db.execute(
            "SELECT * FROM f_users WHERE userid = ?", (self.id,)
        )
    ...
    async def check_collection(self, oid):
        ...
        c = await self.bot.db.execute(
            "SELECT * FROM f_collections WHERE userid = ? AND oid = ?",
            (
                self.id,
                oid,
            ),
    ...
    async def update_collection(self, oid):
        ...
        if (await c.fetchone()) is None:
            await self.bot.db.execute(
                "INSERT INTO f_collections VALUES (?, ?)",
                (
                    self.id,
                    oid,
                ),
    ...
    async def get_collection(self):
        c = await self.bot.db.execute(
            "SELECT COUNT(*) FROM f_collections WHERE userid = ?", (self.id,)
        )
        try:
            amount_caught = (await c.fetchone())[0]
        except Exception:
            amount_caught = 0
    ...
    async def check_trophy(self, caughtoid):
        ...
        if trophyfish is None:
            ...
            await self.bot.db.execute(
                "UPDATE f_users SET trophyoid = ? WHERE userid = ?",
                (
                    caughtoid,
                    self.id,
                ),
            ...
            await self.bot.db.execute(
                "UPDATE f_users SET trophyrodlvl = ? WHERE userid = ?",
                (
                    self.rod_level,
                    self.id,
                ),
            ...
            if data:  # player exists
                ...
                if stored_id != 0:  # they have a review
                    ...
                    await msgtoedit.edit(
                        content=f"Heres a review from {str(self)} ({self.id}):```\n{reviewtext}```"
                    )
                    return "`Success! You've edited your review."
                else:  # they dont have a review
                    thereview = await reviewchannel.send(
                        f"Heres a review from {self.mention} ({self.id}):```\n{reviewtext}```"
                    )
                    await self.bot.db.execute(
                        "Update f_users set reviewmsgid = ? where userid = ?",
                        (
                            thereview.id,
                            self.id,
                        ),
    ...
    async def db_ban(self, reason):
        ...
        await self.bot.db.execute(
            "INSERT INTO bannedusers VALUES (?,?,?)",
            (
                self.id,
                self.name,
                reason,
    ...
    async def db_unban(self):
        c = await self.bot.db.execute(
            "SELECT * FROM f_bans WHERE userid = ?", (self.id,)
        )
        data = await c.fetchone()
        if data:
            await self.bot.db.execute(
                "DELETE FROM bannedusers WHERE userid = ?", (self.id,)
            )
            await self.bot.db.commit()
        if not data:
    ...
    async def update_profilecolor(self, hexstring):
        await self.bot.db.execute(
            "UPDATE f_users SET hexcolor = ? WHERE userid = ?",
            (
                hexstring,
                self.id,
            ),
    ...
    async def get_rod(self):
        c = await self.bot.db.execute(
            "SELECT * FROM f_rods WHERE level = ?", (self.rod,)
        )
        data = await c.fetchone()
    ...
    async def get_net(self):
        c = await self.bot.db.execute(
            "SELECT * FROM f_nets WHERE level = ?", (self.net,)
        )
        data = await c.fetchone()
