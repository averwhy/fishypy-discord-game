...
class HelpCommand(commands.HelpCommand):
    def __init__(self):
        super().__init__(
            command_attrs={
                ...
                "help": "shows this message. you can also do !help [command] for more info on a command.\nexample: !help fish",
            }
        )
        ...
        for cog, cmds in mapping.items():
            ...
            for cmd in cmds:
                if isinstance(cmd, Group):
                    ...
                    subcommands = "".join(_)
                    new_msg = new_msg + (
                        f"[{ctx.prefix}{cmd.name} {subcommands.removesuffix('|')}][{cmd.description}]\n"
                    )
                else:
                    new_msg = new_msg + (
                        f"[{ctx.prefix}{cmd.name}][{cmd.description}]\n"
                    )
        final_msg = new_msg + "\n```"
    ...
    async def send_command_help(self, command: commands.Command):
        ...
        new_msg = new_msg + (
            f"[{ctx.prefix}{command.name}][{command.description}]\n# {command.help}"
        )
        final_msg = new_msg + "\n```"
        await ctx.send(final_msg)
    ...
    async def send_group_help(self, group: commands.Group):
        ...
        final_msg = new_msg + (
            f"[{ctx.prefix}{group.name}][{group.description}]\n# {group.help}\n"
        )
        ...
        if len(final_msg) > 2000:
            return await ctx.send_in_codeblock(
                f"unable to send help, message is too big. please annoy my owner to program this in ({ctx.prefix}support)"
            )
        await ctx.send(final_msg)
