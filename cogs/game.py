...
class game(commands.Cog):
    ...
    async def on_fish_catch(self, player, fish, coins, autofish: bool = False):
        ...
        await self.bot.db.execute(
            "UPDATE f_stats SET totalcoinsearned = (totalcoinsearned + ?)", (coins,)
        )
        self.bot.fishCaughtinsession += 1
        self.bot.coinsEarnedInSession += coins

        await self.bot.db.execute(
            "UPDATE f_users SET coins = (coins + ?) WHERE userid = ?",
            (
                coins,
                player.id,
            ),
        ...
        await self.bot.db.execute(
            "UPDATE f_users SET totalcaught = (totalcaught + 1) WHERE userid = ?",
            (player.id,),
        )
        ...
        if autofish:
            await self.bot.db.execute(
                "UPDATE f_stats SET totalautofished = (totalautofished + 1)"
            )
    ...
    async def do_autofishing(self, ctx, player):
        ...
        while (
            ...
            embed.add_field(
                name="__Coins Earned__",
                value=f"{round((coins_earned - (coins_earned * 0.10)),2)} coins{coin_bonus}",
            )
            ...
            try:
                ...
                self.bot.dispatch(
                    ...
                    player,
                    fish,
                    (coins_earned - (coins_earned * 0.10)),
                    autofish=True,
                )
                await asyncio.sleep(11)

        actual_fishing_time = (
            f"(actual time {humanize.precisedelta(actual_start)})"
            if player.id not in self.bot.autofishers
            else ""
        )
        stopped_or_cancelled = (
            "FINISHED" if player.id in self.bot.autofishers else "CANCELLED"
        )
        try:
    ...
    async def do_fish(self, ctx, player):
        ...
        embed.add_field(
            name="__Coins Earned__",
            value=f"{round(((fish.coins(player.rod_level)) * self.bot.coin_multiplier), 3)} coins{coin_bonus}",
        )
        ...
        self.bot.dispatch(
            ...
            player,
            fish,
            round(((fish.coins(player.rod_level)) * self.bot.coin_multiplier), 3),
            autofish=False,
        )
    ...
    async def on_fish_error(self, ctx, error):
        ...
        if isinstance(error, botchecks.FishNotFound):
            await ctx.send_in_codeblock(
                f"error while fishing, please try again, if this continues please join the support server ({ctx.prefix}support)"
            )
            return
        else:
            await ctx.send_in_codeblock(
                f"Internal error, if this continues please join the support server ({ctx.prefix}support)"
            )

    @commands.group(
        invoke_without_command=True,
        aliases=["af"],
        description="idle fishing, catches are sent in DM",
    )
    async def autofish(self, ctx):
        ...
        summary.append(
            f"notifications level: {player.autofishing_notif} ({fancy_af_notif})"
        )
    ...
    async def start(self, ctx):
        ...
        if is_fishing:
            return await ctx.send_in_codeblock(
                f"you are already autofishing, check [ {ctx.prefix}autofish ] for more info",
                language="ini",
            )
    ...
    async def stop(self, ctx):
        ...
        if not is_fishing:
            return await ctx.send_in_codeblock(
                f"you can't stop autofishing if you aren't autofishing in the first place"
            )
        ...
        try:
            ...
            pass
        return await ctx.send_in_codeblock(
            "autofishing stopped, check your dm's for more information"
        )
    ...
    async def settings(self, ctx):
        ...
        await ctx.send_in_codeblock(
            f"""#______DM NOTIFICATIONS______#
...
[all catches][{l1}](!af set 1)""",
    ...
    @autofish.command(
        name="set", description="change autofishing notification settings"
    )
    async def _set(self, ctx, level: int = None):
        ...
        if level is None:
            return await ctx.send_in_codeblock(
                f"please specify {ctx.prefix}set 1-5 , check [ {ctx.prefix}autofish settings ] for all info",
                language="ini",
            )
        if level > 5:
            return await ctx.send_in_codeblock(
                f"invalid option , check [ {ctx.prefix}autofish settings ] for all info",
                language="ini",
            )

        await self.bot.db.execute(
            "UPDATE f_users SET autofishingnotif = ? WHERE userid = ?",
            (
                level,
                ctx.author.id,
            ),
