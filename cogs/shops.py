...
class shops(commands.Cog):
    ...
    @commands.group(
        ...
        description="shows you the prices of upgrades for rods and nets",
    )
    ...
    async def rods(self, ctx):
        ...
        cur = await self.bot.db.execute(
            "SELECT * FROM f_rods WHERE level = ?", (playeruser.rod_level,)
        )
        ...
        for r in range(6):
            ...
            if step == lvl:
                cur = await self.bot.db.execute(
                    "SELECT * FROM f_rods WHERE level = ?", (step,)
                )
                data = await cur.fetchone()
    ...
    async def nets(self, ctx):
        ...
        cur = await self.bot.db.execute(
            "SELECT * FROM f_nets WHERE level = ?", (playeruser.rod_level,)
        )
        data = await cur.fetchone()
        ...
        for r in range(6):
            ...
            if step == lvl:
                cur = await self.bot.db.execute(
                    "SELECT * FROM f_nets WHERE level = ?", (step,)
                )
                data = await cur.fetchone()
    ...
    @commands.group(
        invoke_without_command=True,
        aliases=["u"],
        description="buy new rods or nets automatically",
    )
    ...
    async def _rod(self, ctx):
        ...
        cur = await self.bot.db.execute(
            "SELECT * FROM f_rods WHERE level = ?", ((playeruser.rod_level + 1),)
        )
        data = await cur.fetchone()
        if data is None:
        ...
        if playeruser.coins < data[2]:
            return await ctx.reply_in_codeblock(
                f"you cannot afford a rod upgrade, you need {round((data[2] - playeruser.coins),2)} more coins"
            )
    ...
    async def _net(self, ctx):
        ...
        cur = await self.bot.db.execute(
            "SELECT * FROM f_nets WHERE level = ?", ((playeruser.net_level + 1),)
        )
        data = await cur.fetchone()
        if data is None:
            return await ctx.reply_in_codeblock(
                "you have the max net level!", language="css"
            )

        if playeruser.coins < data[2]:
            return await ctx.reply_in_codeblock(
                f"you cannot afford a net upgrade, you need {round((data[2] - playeruser.coins),2)} more coins"
            )

        await self.bot.db.execute(
            "UPDATE f_users SET netlevel = (netlevel + 1) WHERE userid = ?",
            (ctx.author.id,),
        )
        await self.bot.db.execute(
            "UPDATE f_users SET coins = (coins - ?) WHERE userid = ?",
