...
class meta(commands.Cog):
    ...
    async def set_prefix(self, ctx, prefix=None):
        ...
        if prefix is None:
            try:
                this_prefix = self.bot.prefixes[ctx.guild.id]
            except KeyError:
                self.bot.prefixes[ctx.guild.id] = self.bot.defaultprefix
                await self.bot.db.execute(
                    "INSERT INTO f_prefixes VALUES (?, ?)",
                    (
                        ctx.guild.id,
        ...
        if prefix.strip() == self.bot.defaultprefix:
            cur = await self.bot.db.execute(
                "SELECT * FROM f_prefixes WHERE guildid = ?", (ctx.guild.id,)
            )
            if (await cur.fetchone()) is None:
                await ctx.send_in_codeblock("the prefix is already !")
            self.bot.prefixes.pop(ctx.guild.id)
            await self.bot.db.execute(
                "DELETE FROM f_prefixes WHERE guildid = ?", (ctx.guild.id,)
            )
            await self.bot.db.commit()
        ...
        if (await cur.fetchone()) is None:
            ...
            await self.bot.db.execute(
                "UPDATE f_prefixes SET prefix = ? WHERE guildid = ?",
                (
                    prefix,
                    ctx.guild.id,
                ),
    ...
    async def on_prefix_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            return await ctx.send_in_codeblock(
                "you must have Manage Guild permissions to change the servers prefix",
                language="ml",
            )
    ...
    async def blacklist(self, ctx, channel=None):
        ...
        guilds_blacklisted_channels = [
            c.name for c in ctx.guild.channels if c.id in self.bot.channel_blacklist
        ]
        ...
        if channel != "all":
            ...
            check_channels = [
                c.id for c in ctx.guild.channels if c.id in self.bot.channel_blacklist
            ]
            if len(check_channels) == (
                len(ctx.guild.channels) - 1
            ):  # all channels in blacklist for some reason
                for c in check_channels:
                    if c == ctx.channel.id:
                        continue
                    self.bot.channel_blacklist.remove(c)
                    await self.bot.db.execute(
                        "INSERT INTO f_blacklist VALUES (?)", (c,)
                    )
                await self.bot.db.commit()
                return await ctx.send_in_codeblock(
                    f"removed {len(check_channels)} channels from blacklist"
                )
            ...
            await self.bot.db.commit()
            return await ctx.send_in_codeblock(
                f"added {len(ctx.guild.channels) - 1} (all) channels to blacklist (except this one)"
            )

        if channel.id in self.bot.channel_blacklist:
            self.bot.channel_blacklist.remove(channel.id)
            await self.bot.db.execute(
                "DELETE FROM f_blacklist WHERE channelid = ?", (channel.id,)
            )
            await self.bot.db.commit()
            return await ctx.send_in_codeblock(
                f"removed #{str(channel)} from blacklist", language="css"
            )

        elif channel.id not in self.bot.channel_blacklist:
            self.bot.channel_blacklist.append(channel.id)
            await self.bot.db.execute(
                "INSERT INTO f_blacklist VALUES (?)", (channel.id,)
            )
            await self.bot.db.commit()
            return await ctx.send_in_codeblock(
                f"added #{str(channel)} to blacklist", language="css"
            )
        else:
            return await ctx.send_in_codeblock(
                f"something went wrong, please try running the command again, if this persists then please join the support server [ {ctx.prefix}support ]",
                language="ini",
            )

    @commands.cooldown(1, 15, BucketType.user)
    @commands.command(
        description="info and stats about bot", aliases=["information", "info"]
    )
...
Database Ping: {db_duration}ms```
    ...
    async def support(self, ctx):  # this sends the fishypy support server link
        em = discord.Embed(
            title="Click me for the support server",
            url="http://discord.gg/HSqevex",
    ...
    async def invite(self, ctx):
        embed = discord.Embed(
            description="[discord.com/api/oauth2/authorize?client_id=708428058822180874&permissions=289856&scope=bot](https://discord.com/api/oauth2/authorize?client_id=708428058822180874&permissions=289856&scope=bot)",
