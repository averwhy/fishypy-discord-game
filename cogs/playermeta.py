...
class playermeta(commands.Cog):
        ...
        if before.name != after.name:
            if await self.bot.usercheck(before.id):
                await self.bot.db.execute(
                    "UPDATE f_users SET name = ? WHERE userid = ?"
                )
                await self.bot.db.commit()

    @commands.check(botchecks.ban_check)
    @commands.command(
        aliases=["prof", "me"], description="player profile, rod level, collection"
    )
    async def profile(self, ctx, user: discord.User = None):  # profile command
        ...
        if playeruser is None:
            return await ctx.send_in_codeblock(
                f"you dont have a profile, use {ctx.prefix}start to get one"
            )
        is_verified = (
            "  <:verified:826441930254057493>"
            if (await self.bot.is_verified(user))
            else ""
        )
        is_dev = (
            "  <:developer:826440754809012244>"
            if user.id in [OWNER_ID, 163247205757616128]
            else ""
        )
        is_bughunter = (
            "  <:bughunter:826483661552484352>"
            if user.id
            in [
                267410788996743168,
                187056629223522305,
        ...
        embed = discord.Embed(
            title=f"{user.name}{is_dev}{is_verified}{is_staff}{is_bughunter}"
        )
        embed.add_field(name="__Coins__", value=round(playeruser.coins, 2))
        embed.add_field(
            name="__Collection__", value=(await playeruser.get_collection())
        )
        embed.add_field(
            name="__Total Caught__", value=f"{playeruser.total_caught} fish"
        )
        playerrod = await playeruser.get_rod()
        embed.add_field(
            name="__Rod__",
            value=f"**{playerrod.name}** (Max length {playerrod.max_length}cm)",
        )
        playernet = await playeruser.get_net()
        embed.add_field(
            name="__Net__",
            value=f"**{playernet.name}** (Autofishing mins: {playernet.mins})",
        )
    ...
    async def removeme(self, ctx):
        ...
        confirm = await ctx.send_in_codeblock(
            "- warning! deleting all of your data is absolutely irreversible.\nyou will lose your entire profile (coins, rods, nets, caught fish, collection, position on leaderboards, etc)\nyou cannot go back. are you sure you want all of your data to be deleted?",
            language="diff",
        )
        did_they = await self.bot.prompt(ctx.author.id, confirm, timeout=15)
        if did_they:
            await self.bot.db.execute(
                "DELETE FROM f_users WHERE userid = ?", (ctx.author.id,)
            )
            await self.bot.db.execute(
                "DELETE FROM f_collections WHERE userid = ?", (ctx.author.id,)
            )
            await self.bot.db.commit()
    ...
    async def trophy(self, ctx, user: discord.User = None):
        ...
        embed.add_field(
            name="__Worth__", value=f"{fish.coins(playeruser.trophy_rod_level)} coins"
        )
        embed.set_footer(
            text=f"#{fish.db_position} out of 16206 catchable fish in the database"
        )
        await ctx.reply(embed=embed)

    @commands.group(
        aliases=["rank", "lb", "leaderboard"],
    ...
    async def top(self, ctx):
        ...
        await ctx.send_in_codeblock(
            f"please specify: {ctx.prefix}top ({','.join([c.name for c in ctx.command.commands])})"
        )
    ...
    async def rod(self, ctx):
        ...
        for r in topusers:
            ...
            table = (
                table
                + f"{step}. {player.name} : {player_rod.name} (lvl {player_rod.level})\n"
            )
            step += 1
        c = await self.bot.db.execute(
            "SELECT * FROM (SELECT userid, RANK() OVER (ORDER BY rodlevel DESC) AS rodlevel FROM f_users) a WHERE userid = ?;",
            (playeruser.id,),
    ...
    async def net(self, ctx):
        ...
        for r in topusers:
            ...
            table = (
                table
                + f"{step}. {player.name} : {player_net.name} (lvl {player_net.level})\n"
            )
            step += 1
        c = await self.bot.db.execute(
            "SELECT * FROM (SELECT userid, RANK() OVER (ORDER BY netlevel DESC) AS netlevel FROM f_users) a WHERE userid = ?;",
            (playeruser.id,),
        )  # i love this statement
        player_pos = (await c.fetchone())[1]
        table = table + f"(your rank: #{player_pos})"
    ...
    async def coin(self, ctx):
        ...
        for r in topusers:
            ...
            step += 1
        c = await self.bot.db.execute(
            "SELECT * FROM (SELECT userid, RANK() OVER (ORDER BY coins DESC) AS coins FROM f_users) a WHERE userid = ?;",
            (playeruser.id,),
        )  # i love this statement
        player_pos = (await c.fetchone())[1]
        table = table + f"(your rank: #{player_pos})"
    ...
    async def _collection(self, ctx):
        ...
        async with ctx.channel.typing():
            ...
            table = ""
            c = await self.bot.db.execute(
                "SELECT userid, COUNT(*) FROM f_collections GROUP BY userid"
            )
            ...
            for r in sortedtopusers:
                ...
                table = (
                    table
                    + f"{step}. {player.name} : ({r[1]}/{num_of_fish[0]}) ({round((r[1]/num_of_fish[0])*100,2)}%)\n"
                )
                step += 1
                if step == 10:
                    break
    ...
    async def caught(self, ctx):
        ...
        cur = await self.bot.db.execute(
            "SELECT * FROM f_users ORDER BY totalcaught DESC"
        )
        topusers = await cur.fetchmany(10)
        step = 1
        table = ""
        for r in topusers:
            ...
            table = (
                table
                + f"{step}. {player.name} : {player.total_caught} total caught fish)\n"
            )
            step += 1
        c = await self.bot.db.execute(
            "SELECT * FROM (SELECT userid, RANK() OVER (ORDER BY totalcaught DESC) AS totalcaught FROM f_users) a WHERE userid = ?;",
            (playeruser.id,),
        )  # i love this statement
        player_pos = (await c.fetchone())[1]
        table = table + f"(your rank: #{player_pos})"
    ...
    @commands.command(
        description="view all fish in your collection with an interactive menu"
    )
    async def collection(self, ctx):
        ...
        c = await self.bot.db.execute(
            "SELECT oid FROM f_collections WHERE userid = ?", (ctx.author.id,)
        )
        data = await c.fetchall()
        pages = menus.MenuPages(
            source=botmenus.CollectionSource(data), clear_reactions_after=True
        )
